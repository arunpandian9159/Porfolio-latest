...
const GitHubActivity = ({ username, className = '' }) => {
  ...
  const weeks = useMemo(() => {
    if (!data?.contributionData) return [];
  ...
  const handleMouseEnter = (e, day) => {
    ...
    const containerRect = e.target.closest('.github-activity-container')?.getBoundingClientRect();
            ...
            <div key={weekIndex} className="flex flex-col gap-[3px]">
              {week.map((day, dayIndex) => (
                <div
                  ...
                  } ${day ? 'hover:ring-1 hover:ring-white/30 cursor-pointer' : ''}`}
                  onMouseEnter={(e) => handleMouseEnter(e, day)}
                  onMouseLeave={handleMouseLeave}
                  aria-label={day ? `${day.count} contributions on ${formatDate(day.date)}` : undefined}
                />
              ))}
            </div>
          ))}
